...
# Define the paths
dock_path = Path.home() / "meet-u/docking_config_files"      # Your docking folder path /doc_config
ligend_folder_path = Path.home() / "meet-u/ligends_database"     # Folder containing the ligands /ligends
receptor_path = "/home/imran/meet-u/target_receptor/4YPX_prot.pdbqt"  # Receptor path for docking
docking_results_path = Path.home() / "meet-u/docking_results"  # /docking_result
...
for ligand_file in ligend_folder_path.glob("*.pdbqt"):
    ...
    with open(config_path, "w") as f:
        ...
        f.write(f"ligand = /home/imran/meet-u/ligends_database/{ligand_file.name} \n")  # Write ligand path /ligneds
